...
class Country:    
    ...
    def __implement_data(self):
        ...
        if data:
            ...
            self.dependent = not classification.get('dependency', False)

            # Capitals
            capital_list = data.get('capitals')
            if isinstance(capital_list, list) and capital_list:
                self.capital = [cap.get('name') for cap in capital_list if isinstance(cap, dict) and cap.get('name')]
            else:
                self.capital = []
            ...
            if isinstance(area_obj, dict):
                self.area = area_obj.get('kilometers')
            else:
                self.area = None
            ...
            if isinstance(currencies_data, list):
                ...
                self.currencies_symbol = [c.get("symbol") for c in currencies_data.values() if isinstance(c, dict) and c.get("symbol")]

            # Languages
            self.idioms = []
            languages_data = data.get("languages", [])
            lang_list = languages_data.values() if isinstance(languages_data, dict) else languages_data
            for item in lang_list:
                if isinstance(item, dict):
                    name_field = item.get("name")
                    if isinstance(name_field, list) and name_field:
                        self.idioms.append(name_field[0])
                    elif isinstance(name_field, str):
                        self.idioms.append(name_field)
                elif isinstance(item, str):
                    self.idioms.append(item)
    ...
    def __get_data(self):
        try:
            url = "https://guess-backend-lbjx.onrender.com/country"
            ...
            if response.status_code == 200:
                data = response.json()
                if (
                    data.get("summary") and
                    data["summary"].get("data") and
                    data["summary"]["data"].get("objects") and
                    len(data["summary"]["data"]["objects"]) > 0
                ):
                    return data["summary"]["data"]["objects"][0]
            return None
    ...
    def get_tips(self, level):
        ...
        dependency_status = "Não (País Soberano)" if self.dependent else "Sim"
        ...
        borders_text = ", ".join(nomes_fronteiras) if self.borders else "Nenhuma (Ilha)"
        ...
        if level == 1:
            ...
            ready_tips = [tip for tip in tips if "Não informado" not in tip]
            return random.choice(ready_tips) if ready_tips else "Dica indisponível para este país."
            ...
            ready_tips = [tip for tip in tips if "Não informado" not in tip and "Nenhuma (Ilha)" not in tip]
            return random.choice(ready_tips) if ready_tips else "Dica indisponível para este país."
    ...
    def __str__(self):
        ...
        return (
            f"\n"
            f"{'='*40}\n"
            f" 🌍 INFORMAÇÕES SOBRE: {self.name.upper()} ({self.code.upper()}) - {flag_emoji}\n"
            f"{'='*40}\n"
